...
def convert_input_text_to_tensor_dataset(text,
                                         pred_config,
                                         args,
                                         cls_token_segment_id=0,
                                         pad_token_segment_id=0,
    ...
    for line in lines:
        line = line.strip()
        if not line:  # last line may be empty
            continue
        tokens = tokenizer.tokenize(line)
        # Account for [CLS] and [SEP]
        special_tokens_count = 2
...
def predict(text, pred_config):
    ...
    dataset = convert_input_text_to_tensor_dataset(text, pred_config, args)
    ...
    for batch in tqdm(data_loader, desc="Predicting"):
        ...
        with torch.no_grad():
            ...
            if preds is None:
                ...
                preds = np.append(preds, logits.sigmoid().detach().cpu().numpy(), axis=0)
